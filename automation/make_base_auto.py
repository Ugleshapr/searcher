...
def _is_wanted_doc(category: str, title: str) -> bool:
    ...
    has_sert   = re.search(r"сертификат", s, flags=re.IGNORECASE) is not None
    return bool(has_catalog or has_re_full or has_re_abbr or has_passport or has_oprosny or has_sert)
...
def build_site_link_map_from_products(search_dir: Path) -> dict:
    """
    Читает products.csv (без заголовков) и возвращает art -> URL (берём только 0 и 13 колонки).
    0-й столбец — артикул/ID, 13-й — URL сайта.
    """
    ...
    try:
        ...
        pr = pd.read_csv(
            ...
            engine='python',
            usecols=[0, 13],
            error_bad_lines=False,         # устаревшее, но работает
            warn_bad_lines=True,
            quotechar='"',
            quoting=csv.QUOTE_MINIMAL,
            escapechar='\\',
    ...
    if pr.empty:
        print(f"[i] В {prod_path.name} нет данных — «Сайт ссылка» пропущена.")
        return {}

    pr.columns = ["Артикул", "URL"]
    pr["Артикул"] = pr["Артикул"].map(clean_article)
    pr["URL"] = pr["URL"].astype(str).str.strip()

    pr = pr[(pr["Артикул"] != "") & pr["URL"].str.match(r"^https?://", na=False)]
    m = dict(zip(pr["Артикул"], pr["URL"]))
...
def _normalize_specs(text: str) -> str:
    """
    Удаляет ведущий код вида `272 |` в КАЖДОЙ строке и чистит пробелы.
    Сохраняет порядок строк и переносы.
    """
...
def attach_specs_from_enriched(base_df: pd.DataFrame, search_dir: Path) -> pd.DataFrame:
    ...
    enr = enr.iloc[:, :3].copy()
    enr.columns = ["Артикул", "НаименованиеEnriched", "Характеристики"]
...
def enrich_names_with_products(base_df: pd.DataFrame, search_dir: Path) -> pd.DataFrame:
    """
    base_df: 'Наименование','Артикул','Цена'
    products.csv: без заголовков; 0 = Артикул/ID, 2 = Наименование (в одинарных кавычках)
    """
    prod_path = _find_products_file(search_dir)
    if not prod_path:
    ...
    if pr.empty:
        print(f"[i] В {prod_path.name} нет данных — обогащение пропущено.")
        return base_df

    pr.columns = ["Артикул", "НовоеИмя"]
    pr["Артикул"] = pr["Артикул"].map(clean_article)
    pr["НовоеИмя"] = pr["НовоеИмя"].map(strip_outer_single_quotes)

    prod = pr[(pr["Артикул"] != "") & pr["НовоеИмя"].astype(str).str.strip().ne("")] \
