...
class PriceListSearchApp {
  constructor() {
    ...
    this.translitMap = {
      ...
      'з':'z','и':'i','й':'y','к':'k','л':'l','м':'m','н':'n','н':'h','о':'o',
      'п':'p','р':'p','с':'s','т':'t','у':'u','ф':'f','х':'kh','ц':'ts',
      'ч':'ch','ш':'sh','щ':'sch','ъ':'','ы':'y','ь':'','э':'e','ю':'yu','я':'ya',
      'a':'а','b':'б','v':'в','g':'г','d':'д','e':'е','f':'ф','h':'x',
...
window.Tips.bindIndex(menu, tips, { onOpenDetail: openDetail });
  ...
  // «Тихий» старт: результатов ещё нет, но данные уже загружены —
  // дёрнем событие, чтобы показать "Что нового".
...
title = parseTitle(md);
// Удаляем первую строку (# Заголовок), чтобы не дублировалась в теле
const cleaned = md.replace(/^#\s+.+(?:\r?\n)+/, '');
html = mdToHtml(cleaned);
    } catch {
      title = 'Что нового';
