...
function normalizeFacetValue(raw, dict, specId) {
  ...
  async function buildIndex(articles) {
    ...
    if (!_valuesCache) {
      _valuesCache = await parseCSV('src/addons/products_spec_values.csv', true);
    }
    const values = _valuesCache;
    ...
    for (const row of values) {
      ...
      let arr = perArt.get(art);
if (!arr) {
  arr = [];
  perArt.set(art, arr);
}
arr.push({ spec_id: sid, value, dict });
  ...
  function initToggle() {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!document.body.classList.contains('is-filter-mode')) {
        // Открытие: только говорим FilterAddon открыть,
        // он сам бросит filter:opened, а там уже вызовут FilterPanel.open
        window.FilterAddon?.open?.();
      } else {
        // Закрытие: говорим FilterAddon закрыть,
        // он бросит filter:closed, а там уже вызовут FilterPanel.close
